...
<%
    Garde[] list=(Garde[])request.getAttribute("list");
%>
<body>
    <header>
        <a href="#" class="logo">Animatronic</a>

        <form class="form form-inline" action="<%= request.getContextPath() %>/Garde" method="get">
            <h3>Voir les gardes libre pour ces dates</h3>
            <label class="form-label" for="debut">Debut</label>
            <input class="form-control" type="datetime-local" id="debut" name="debut" required>
            <label class="form-label" for="fin">Fin</label>
            <input class="form-control" type="datetime-local" id="fin" name="fin" required>
            <button type="submit" class="btn btn-default">Valider</button>
        </form>

        <ul class="nav-list">
            <li>
                <a href="Sitting" class="nav-link">
            ...
            <% if (session.getAttribute("user")!=null) { %>
                <li>
                    <a href="<%= request.getContextPath() %>/pagesAdmin/logout.jsp" class="nav-link">
    ...
    <div class="container">
            ...
            <table class="table">
                ...
                <% for (int i=0; i!=list.length; i++) { %>
                    <tr>
                        <td><%= list[i].getId() %></td>
                        <td><%= list[i].getNom() %></td>
                        <td><a href="FormGarde?id=<%= list[i].getId() %>" class="btn btn-success">Edit</a></td>
                        <td><a href="Garde?mod=d&id=<%= list[i].getId() %>" class="btn btn-danger">Delete</a></td>
                    </tr>
                <% } %>
            </table>
        <% } %>
        ...
        <% if (session.getAttribute("user")!=null) { %>
            <a href="FormGarde" class="nav-link" id="more">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                    <path
